...
pwd <- "~/Desktop/Dropbox/2019_Goldsby_et_al_Entrenchment_MS/Entrenchment_processed_data_for_figures/Figure_6/Figure_6_data/020/020_trans/"
setwd(pwd)
folders <- list.dirs(path = ".", full.names = TRUE, recursive = FALSE)


for (working_dir in folders) {
  ...
  for (file in filenames[1:length(filenames)]) {
    ...
    for (i in unique(genomes$reversion_mut_ID)) {
      ...
      for (j in unique(genomes$iteration[genomes$reversion_mut_ID==i])) {
        ...
        print(paste(strsplit(working_dir, split="/")[[1]][2], i, "rep:", j, Sys.time(), sep=" "))
    ...
    write.csv(new_lod_fitness_df, paste(strsplit(working_dir, split="/")[[1]][2], "_lod_fitness_trans_30APR24.csv", sep=""))
...
pwd <- "~/Desktop/Dropbox/2019_Goldsby_et_al_Entrenchment_MS/Entrenchment_processed_data_for_figures/Figure_6/Figure_6_data/020/020_end/"
setwd(pwd)
folders <- list.dirs(path = ".", full.names = TRUE, recursive = FALSE)
